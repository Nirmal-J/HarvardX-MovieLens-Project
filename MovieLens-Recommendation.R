...
# Now, we split the 'edx' set in 2 parts: the training set and the test set. 
# The model building is done in the training set.
# The test set is used to test the model.
# When the model is complete, we use the 'validation' set to calculate the final RMSE.
...
# Data exploration
################################
# Before creating the model, we need to understand the features of the rating data set.
...
# The dataset is very large and the data is stored in a data frame. 
# First, let's transform this data to matrix and perform  dimension reduction 
# train <- as.matrix(edx)
...
library(lubridate) # Helpful for building the date format
...
################################
# recosystem is a package for recommendation system using Matrix Factorization. 
# High performance multi-core parallel computing is supported in this package.
...
# ------------------
# This code takes a few minutes to run, but it works. Make sure you adjust you memory limit to 10500.
